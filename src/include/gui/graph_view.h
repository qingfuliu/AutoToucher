...
public:
    ...
    /* initializing */
    /**
     * @brief: 管理信号。连接View所接受的鼠标信号
     * @param view :该对象所在的MyGraphicsView
     */
    ...
    /**
     * @brief: 选中的时候执行
     * @details: 将state设置为ON_SELECTED，并且触发selected(this)信号
     */
    void select();
    /**
     * @details:
     *      1、visited为false：将state的ON_VISIT状态取消掉，根据是否被选中来设置颜色
     * @param visited
     */
...
public slots:
    ...
    /**
     * @details:    具体逻辑如下：
     *     1、如果state在PREPARING状态下（正在播放起始动画），此次点击视为无效
     *     2、如果state在ON_LEFT_CLICK或者ON_RIGHT_CLICK状态下，此次点击视为无效
     *     3、调用itemShow();
     *     4、如果是被点击的对象，那么释放emit selected(this)函数（连接setSel，主要是改变View状态），
     *        设置state |= ON_LEFT_CLICK，调用onClickEffect(将对象缩小);
     *     5、如果不是被选中的对象，但是当前对象之前被选中了，那么设置state &= ~ON_SELECTED;设置颜色this->setBrush(regBrush);调用visit(false);
     *
     * @param position
     */
